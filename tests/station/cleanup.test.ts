...
describe('landedGlobalIds', () => {
  it('collects ids from both mcpJson and localScope of all projects lastApplied', () => {
    const s = emptyState();
    s.lastApplied['/a'] = { mcpJson: { exa: { command: 'exa' } }, localScope: { firecrawl: { command: 'npx' } }, skills: [], plugins: [], snippets: [], bundles: [] };
    s.lastApplied['/b'] = { mcpJson: {}, localScope: { memory: { command: 'm' } }, skills: [], plugins: [], snippets: [], bundles: [] };
...
describe('globalCleanupStatus', () => {
  it('splits top-level ids into eligible (landed) and blocked (not landed)', () => {
    const s = emptyState();
    s.lastApplied['/a'] = { mcpJson: {}, localScope: { firecrawl: { command: 'npx' } }, skills: [], plugins: [], snippets: [], bundles: [] };
